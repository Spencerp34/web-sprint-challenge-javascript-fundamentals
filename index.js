...
const zooAnimals = [
 ...
 function animalNames(){
  ...
  const plaque = function(animal) {
    scienceName.push( `name: ${animal.animal_name}, scientific: ${animal.scientific_name}`)
  }

  zooAnimals.forEach(plaque);
  return scienceName;
}
...
function lowerCaseNames(){
  ...
  const smallLetters = function(animal){
    return animal.animal_name.toLowerCase()
  }

  return zooAnimals.map(smallLetters);
}
...
function lowPopulationAnimals(){
  let result = [];
  const popCount = function(animal){
  ...
  result = zooAnimals.filter(popCount)
  return result;
 
  }
console.log('Topic 2 request 3', lowPopulationAnimals());
...
// console.log(copy(zooAnimals))

function USApop(copycb){
  const getTotal = copycb.reduce(function(accumulator, index){
...
const myCube = new CubeMaker({'length': 5})
